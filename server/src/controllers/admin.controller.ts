...
export const listServicesAdmin = asyncHandler(async (_req: Request, res: Response) => {
...
export const listHoursAdmin = asyncHandler(async (_req: Request, res: Response) => {
...
export const upsertHours = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data, error } = await supabase
    .from("business_hours")
    .upsert(input, { onConflict: "day_of_week" })
    .select("*")
    .single();
  if (error) throw new ApiError(500, error.message);
  res.json(data);
...
export const listStaffAdmin = asyncHandler(async (_req: Request, res: Response) => {
  const { data, error } = await supabase.from("staff").select("*").order("display_order");
  if (error) throw new ApiError(500, error.message);
  res.json(data);
});
...
export const listAppointmentsAdmin = asyncHandler(async (_req: Request, res: Response) => {
  const { data, error } = await supabase
    .from("appointments")
...
export const listKnowledgeAdmin = asyncHandler(async (_req: Request, res: Response) => {
  const { data, error } = await supabase.from("ai_knowledge").select("*").order("topic");
  if (error) throw new ApiError(500, error.message);
  res.json(data);
});
...
export const createKnowledge = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data, error } = await supabase.from("ai_knowledge").insert(input).select("*").single();
  if (error) throw new ApiError(500, error.message);
  res.status(201).json(data);
});
...
export const updateSalonProfile = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data: existing } = await supabase.from("salon_profile").select("id").limit(1).maybeSingle();
  if (!existing) throw new ApiError(404, "Salon profile not found");
  const { data, error } = await supabase
    .from("salon_profile")
    .update(input)
    .eq("id", existing.id)
...
// ---------------------------------------------------------------------------
export const listConversationsAdmin = asyncHandler(async (_req: Request, res: Response) => {
  const { data, error } = await supabase
