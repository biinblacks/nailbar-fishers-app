...
import { resolveSalon } from "./middleware/tenant.middleware.js";
...
app.use(
  cors({
    ...
    credentials: true,
    allowedHeaders: ["Content-Type", "Authorization", "x-salon-slug"],
  })
);
...
// Every /api route is scoped to one salon (multi-tenant).
...
app.listen(env.port, () => {
  logger.info(`Nail Bar API listening on port ${env.port} (default salon: ${env.defaultSalonSlug})`);
});
